//
//  MyGarmentsDetailController.swift
//  StoreApp
//
//  Created by Abhilesh Halarnkar on 24/11/16.
...
class MyGarmentsDetailController: BaseDetailController {
    ...
    override func prepareDataSources(shouldReload: Bool = true) {
        ...
        if product != nil {
            ...
            titleCellDetail.cellSize = CGSize.init(width: self.view.frame.width , height: 10 + (product?.name?.heightWithConstrainedWidth(width: self.view.frame.width - 16, font: UIFont.systemFont(ofSize: 22)))!)
            ...
            if idList.contains(Int(product!.id)) {
                ...
                let textCellDetail = CellDetail()
                textCellDetail.cellSize = CGSize.init(width: self.view.frame.width ,height: 50)
                textCellDetail.cellType = .Text
                sectionDetail.sectionDataList.append(textCellDetail)
            }
    ...
    @IBAction func btnBuyTapped(_ sender: UIButton) {
        if product?.default_variant != nil {
            ...
            let dict: [String:String] = ["name":(product?.selectedColor)!]
...
extension MyGarmentsDetailController: UITableViewDelegate {
...
extension MyGarmentsDetailController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .Text:
            ...
            cell.textChanged = { (text) in
                self.product?.selectedColor = text
            }
            return cell
