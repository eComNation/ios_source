...
open class SKPhotoBrowser: UIViewController, UIScrollViewDelegate {
    ...
    open func tilePages() {
        ...
        var lastIndex  = Int(floor(((visibleBounds.maxX - 1) - 10 * 2) / visibleBounds.width))
        if firstIndex < 0 {
            firstIndex = 0
        }
        ...
        if let titles = actionButtonTitles {
            ...
            if photo.caption != nil {
                if let shareExtraCaption = shareExtraCaption {
                    activityItems.append(((photo.caption as AnyObject) as! String) + shareExtraCaption as AnyObject)
                } else {
